...
def cou_list():
    cou = pd.read_csv(file_pro, index_col='교수번호')
    for idx, row in cou.iterrows():
...
def cou_list():
    ...
    for idx, row in merge.iterrows():
        print(row['강좌번호'], row['강좌이름'], row['강의실'], row['교수이름'], end=' ')
        print(row['강의시수'])
